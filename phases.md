...
- [x] `datagen/`: generates (a) a catalog of ~50 users with segments (commuter, traveler, nightlife, etc.), (b) ~40 ad campaigns across two demand types — `auction` (bid, budget) and `guaranteed` (impression goal, flight dates) — with categories (food, retail, entertainment, travel, transit) and audience targeting (`targeted_audiences` — added during Phase 5 planning as a flagged amendment, to give Phase 5's audience routing rule a campaign-side data source; empty for nearly every campaign, exactly two deterministically/thematically targeted; see `PROGRESS.md`), (c) 30 days of historical impression/click logs where clicks follow planted preferences (e.g. traveler segment clicks travel ads at 3x base rate; nightlife segment clicks food/entertainment at night), and (d) 30 days of ride history with a planted segment-dependent frequency (`rides.parquet` — added during Phase 1 planning as a flagged amendment, to give Phase 1's `user_rides_per_week` feature a real data source; see `PROGRESS.md`). Seeded and deterministic.
...
**Build:**
- [x] Event schema: session events (`session_start`, `destination_entered` {category, geo}, `ride_type_selected`, `app_screen_view`).
- [x] `stream_features/consumer.py`: consumes from Redpanda, maintains windowed session aggregates (current destination category, screens viewed last 10 min, session active flag), writes to Redis with TTL (30 min). Registry entries for these features declare freshness SLA in seconds.
- [x] **Streaming handler interface (extensibility contract, symmetric to the Phase 1 batch-job interface):** each event type is handled by a registered handler declaring `event_type()`, `outputs()` (registry feature names), and `update(event, state) -> feature writes`. The consumer framework owns consumption, deserialization, dispatch-by-type, Redis writes, TTLs, and metrics — handlers own only the feature logic. Adding a feature from a new product event = one handler module + registry entries + (if new) the event schema; zero edits to the consumer core or any downstream service. The live model ignores new features until a future version's config includes them.
- [x] **Unknown-event tolerance (required):** events with unrecognized types are skipped, counted, and surfaced in `/metrics` as `unknown_events_by_type` — never a crash, never a stall. This is the forward-compatibility posture for product teams shipping events before this pipeline handles them; the unknown-event counter is also the discovery mechanism for new signals worth building features from.
- [x] Consumer `/metrics`: events consumed, processing latency, and lag (latest event timestamp minus last processed event timestamp).
- [x] **Two event sources, by design — build both:**
...
**Build:**
- [x] `ranking/train.py`: builds training data from historical impressions via **point-in-time joins** against the offline store (features as of impression time — never current values); trains a pCTR model; outputs a versioned artifact directory `models/pctr/v{N}/` containing model file, feature list (pinned to registry names), training-data date range, and an eval report (AUC + calibration curve on a time-split holdout).
- [x] **Model artifact contract (decomposability):** every artifact is loadable as an object exposing `predict(feature_dict) -> float` plus `feature_names()`. The scorer speaks only this interface — it never imports an ML library directly. A model version is opaque: the algorithm, hyperparameters, and feature list are internal details recorded in the artifact's metadata (`training_config.json`), not part of its identity. Training runs take a config file (which happens to name an algorithm, among other settings); the output is simply the next version. The registry, scorer, promotion flow, and A/B arms deal only in versions — v3 might be logistic regression and v4 XGBoost with a different feature set, and nothing downstream knows or cares. Comparing any two versions is one operation: eval reports on the identical holdout offline, arms in the A/B online.
- [x] Minimal model registry: `models/registry.json` mapping logical name → version → path + status (`candidate` | `live` | `retired`). `ranking/promote.py v{N}` flips live pointer; rollback = promote previous version.
- [x] `ranking/scorer.py`: library loaded by the ad server; loads whatever version is `live`, refuses to load if the model's feature list contains names absent from the feature registry.
- [x] Sanity eval: model's learned lifts directionally match the planted signal from Phase 0 (traveler × travel ads etc.).
- [x] **Feature assembly contract (user/ad separation):** four feature classes with distinct handling — *user* features (fetched once per request), *ad* features (fetched per candidate; cacheable in-process since the catalog is small and refreshed on materialization), *context* features (request-scoped: hour of day, slot — computed inline, never stored), and *cross* features (`x_` prefix: pure functions of the other three, e.g. `x_user_ctr_in_ad_category`; never stored, never in the online store). Cross functions live in one shared module (`common/crosses.py`) imported by BOTH `train.py` and the scorer — a single implementation is the defense against training-serving skew in the join. A model's pinned feature list may reference all four classes; the assembly step resolves each by class. (Only `hour_of_day` is used as the concrete context feature — "slot" has no generated concept in this project's datagen, so it's not invented. `scorer.py` imports `common/crosses.py` to *validate* a model's pinned `x_`-prefixed names against it, not to compute them — actual cross-feature computation happens in `ranking/assemble.py`, used by `train.py`'s offline path and, eventually, Phase 5's serving path, which is where the single-implementation defense against skew actually lives.)

**Acceptance criteria:**
- [x] 1. `make train` is fully reproducible: same data + seed → same eval metrics.
- [x] 2. Eval report generated per version; AUC beats a popularity-only baseline; planted-signal check passes.
- [x] 3. Leakage test: a deliberately leaked feature (post-click information) added in a test raises AUC suspiciously — test documents the detection, then asserts the production feature list excludes it.
- [x] 4. Promote v2, roll back to v1 — scorer follows the live pointer without code changes.
...
**Acceptance criteria:**
- [~] 1. Load test at 50 RPS: p99 ≤ 100ms; per-stage latencies visible. **Deviation:** built and measured (`ab`, not a Python client — every Python-based load client tried measured multi-second, unreliable tails against this exact server, a GIL-contention artifact; `ab`'s numbers are the trustworthy ones), but actual p99 is 150-300ms, not ≤100ms, even after real tuning (sklearn's internal OpenMP/BLAS thread-pool oversubscription fixed via `threadpoolctl.threadpool_limits(1)` — a single v2 scorer call went from ~2300ms to ~23ms average under concurrent load; multi-worker uvicorn; raised AnyIO threadpool cap; GC threshold/freeze tuning). Per-stage latencies are visible in `/metrics`. See `PROGRESS.md` for the full trail — same category of local-machine-contention deviation as Phase 3's AC4.
- [x] 2. **Failure-mode tests (required):** (a) stop Redis → popularity fallback serves, rung logged; (b) bidder latency forced above timeout → auction proceeds internally, timeout logged; (c) model artifact removed → house ad, rung logged. No request returns 500 in any scenario.
- [x] 3. Guaranteed-delivery test: a campaign with a 1,000-impression goal over a simulated 10-day flight ends within ±10% of goal under steady traffic, and wins arbitration when behind schedule.
- [x] 4. Concurrency test demonstrating the pacing overshoot; documented.
- [x] 5. Two model versions live under A/B; assignment is deterministic per user and logged; arms receive ~50/50 traffic over 1,000 requests.
- [x] 6. End-to-end freshness test: emit `destination_entered{category: travel}` → within 3 seconds, `/serve` for that user reflects the real-time feature in the logged feature set (and, given planted signal, shifts scores toward travel ads). **Note:** the score shift comes from a small, explicit, non-learned rule-based boost in `scoring.py` (`DESTINATION_MATCH_BOOST`), not a trained model coefficient — neither v1 nor v2 uses `user_current_destination_category` as a pinned input, since there's no historical log of past Redis state to train against (Phase 4's documented limitation). Discussed and confirmed with the user before implementing.
- [x] 7. Audience test: a campaign targeting `frequent_airport_travelers` never serves to non-members (asserted over 1,000 replayed requests, exclusions visible in the decision log with definition_version); the same user still receives non-targeted campaigns from other categories — proving audiences gate eligibility, not relevance. Uncovered and fixed a real bug while building this: the popularity-degradation fallback bypassed audience gating entirely (couldn't reach feature_service to confirm membership, so didn't check at all) — fixed to exclude every audience-targeted campaign unconditionally in that path.
...
**Build:**
- [x] `ops/reconcile.py`: batch job comparing decision-log impressions against pacing counters and campaign delivery; reports discrepancies (there will be some, per Phase 5's known flaw) with tolerances.
- [x] Retraining path: `make retrain` builds training data from decision logs (not the original synthetic history), producing a candidate model version with an eval report comparing it to live. **Forward pointer from Phase 4:** real-time (Redis) features were structurally excluded from every Phase 4 model config — no historical log of past Redis state existed to join against synthetic-history impressions. Once the decision log (Phase 5) is capturing per-request feature values (including real-time ones) at serving time, this is the natural point to add Redis-backed features (e.g. `user_session_active`, `user_current_ride_type`) to a pinned feature list; `ranking/train.py` needs no code change for this, only a feature-list config addition. Built: `ranking/retrain.py`'s `RETRAIN_CONFIG` adds `user_session_active` (cast to 0/1) to `V1_CONFIG`'s feature set — required a flagged amendment to `adserver/features.py` (tagged phase-5) to actually fetch it at serving time, since it had been dropped from `USER_FEATURE_NAMES` for latency reasons. `user_current_ride_type` and `user_current_destination_category` stay out (the latter is categorical, needs encoding infra this module doesn't otherwise have). **Second forward pointer from Phase 4:** static ad attributes (`category` as a direct feature, `advertiser_name`) also aren't used by any Phase 4 model config — a deliberate baseline-first choice, not an oversight. Worth exploring here too: register them in `common/registry.yaml`, add a new model config referencing them, and compare its eval report against the live version's on the identical holdout to see how much AUC improves. **Not pursued this phase** — out of the approved Phase 6 plan's scope (which scoped retraining to the real-time forward pointer only); remains open for a future pass.
- [x] Experiment readout: small notebook/script computing per-arm CTR with confidence intervals from the decision log, joined on logged assignment. README note (not code) on why observational CTR comparisons across arms are trustworthy here (randomized assignment) and what would break trust (assignment drift, logging loss) — plus a paragraph mapping this to incrementality/lift measurement for brand campaigns, which is out of build scope. Built as `ops/readout.py` + `ops/README.md`.

**Acceptance criteria:**
- [x] 1. Reconciliation runs and quantifies the serve-vs-counter discrepancy; report readable.
- [x] 2. `make retrain` produces a promotable candidate trained purely on logged serving decisions.
- [x] 3. Readout script yields per-arm CTR + CIs; a planted model-quality difference between arms is detectable at n=5,000 requests.
