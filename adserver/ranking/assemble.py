...
from adserver.common.crosses import x_user_ctr_in_ad_category, x_user_in_audience_matching_ad_category
...
def _assemble(
    ...
    ad_category: str,
    audiences: dict[str, "AudienceDef"] | None = None,
) -> dict[str, Any]:
    merged = {**user_features, **ad_features}
    merged["x_user_ctr_in_ad_category"] = x_user_ctr_in_ad_category(user_features, ad_category)
    # audiences is optional (default None -> skip) so Phase 4's existing
    # call sites (train.py, its tests) are unaffected by this Phase 5
    # addition — flagged, additive change to already-tagged phase-4 code.
    if audiences is not None:
        merged["x_user_in_audience_matching_ad_category"] = x_user_in_audience_matching_ad_category(
            user_features, ad_category, audiences
        )
    return merged


def from_offline_row(
    ...
    return _assemble(user_features, ad_features, ad_category, audiences)
